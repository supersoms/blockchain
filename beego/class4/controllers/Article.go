...
func (this *ArticleController) ShowArticleList() {
	...
	if typeName == "" {
		...
		//Filter("ArticleType__TypeName", typeName)函数是指定过滤结构体ArticleType中的字段名TypeName对应表中相同的字段为typeName的数据,比如过滤A,那就显示A的数据
	...
	start := pageSize * (pageIndex - 1)                                //计算起始位置,就是第1页为0
	...
	this.Data["userName"] = this.GetSession("userName") //传递登录用户名到UI页面上进行显示
	...
	this.LayoutSections["contentHead"] = "title_article_list.html"
...
func (this *ArticleController) ShowAddArticle() {
	...
	this.Layout = "layout.html"
	this.LayoutSections = make(map[string]string)
	this.LayoutSections["contentHead"] = "title_article_add.html"
...
func (this *ArticleController) ShowArticleDetailContent() {
	...
	//3获取插入对象
	userName := this.GetSession("userName") //传递登录用户名到UI页面上进行显示
	user := models.User{}
	user.UserName = userName.(string)
	orm.Read(&user, "UserName")
	//4.多对多插入
	_, err = m2m.Add(&user)
	if err != nil {
		beego.Error("插入读者数据失败")
		return
	}
	...
	//TODO 查询最近浏览文章的读者
	//方法1
	//orm.LoadRelated(&article,"Users")
	//方法2
	var users [] models.User
	//Articles__Article__Id 字段名(它是user表(结构体)中的字段)__表名__字段名
	orm.QueryTable("User").Filter("Articles__Article__Id", id1).Distinct().All(&users)
	beego.Info(article)

	//3 传递数据给视图
	this.Data["users"] = users //最近浏览文章的阅读者
	this.Data["article"] = article
	this.Data["userName"] = this.GetSession("userName") //传递登录用户名到UI页面上进行显示
	this.Layout = "layout.html"
	this.LayoutSections = make(map[string]string)
	this.LayoutSections["contentHead"] = "title_article_detail.html"
...
func (this *ArticleController) ShowEditArticle() {
	...
	//4 跳转到编辑文章UI界面
	this.Layout = "layout.html"
	this.LayoutSections = make(map[string]string)                  //创建一个map对象
	this.LayoutSections["contentHead"] = "title_article_edit.html" //将title_article_edit.html赋值给contentHead,layout.htm就可以根据key contentHead取值
...
func (this *ArticleController) AddArticleType() {
	...
	this.Layout = "layout.html"
	this.LayoutSections = make(map[string]string)
	this.LayoutSections["contentHead"] = "title_article_add_type.html"
...
func (this *ArticleController) HandlerAddArticleType() {
	...
	this.Redirect("/article/addArticleType", 303)
}

func (this *ArticleController) DeleteArticleType() {
	articleTypeId := this.GetString("id")
	id, _ := strconv.Atoi(articleTypeId)
	if id == 0 {
		beego.Error("")
		return
	}
	...
	//默认是级联删除,相关的外表(Article)信息都会删除掉
	orm.Delete(&articleType)
